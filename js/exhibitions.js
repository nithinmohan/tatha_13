(function($) {
					...
					function( dfd ) {
						...
						$el.stop()
						   .animate( animParam, settings.animSpeed, settings.animEasing, function() {
								if( !expanded )
									$el.find('.eva-content').fadeIn( settings.contentAnimSpeed );
						   })
						   .find('.eva-title')
						   .stop()
						   .animate({
						...
						if( i === $nextSlice.index() ) {
							...
							$slice.stop()
						          .animate({
										...
										$slice.find('.eva-content').fadeIn( settings.contentAnimSpeed );
								  })
								  .find('.eva-title')
								  .stop()
								  .animate({
			...
			init 		: function( options ) {
				...
				if( this.length ) {
					...
					return this.each(function() {
						...
						// click event
						$slices.bind('click.evaccordion', function(e) {
						...
						// navigation events
						$navNext.bind('click.evaccordion', function(e){
							aux.navigate( 1, $slices, $navNext, $navPrev, settings );
						});
						
						$navPrev.bind('click.evaccordion', function(e){
							aux.navigate( -1, $slices, $navNext, $navPrev, settings );
						});
						
						// adds events to the mouse
						$el.bind('mousewheel.evaccordion', function(e, delta) {
							if(delta > 0) {
								aux.navigate( -1, $slices, $navNext, $navPrev, settings );
							}	
							else {
	...
	$.fn.evaccordion = function(method) {
		if ( methods[method] ) {
			return methods[method].apply( this, Array.prototype.slice.call( arguments, 1 ));
		} else if ( typeof method === 'object' || ! method ) {
			return methods.init.apply( this, arguments );
		} else {
			$.error( 'Method ' +  method + ' does not exist on jQuery.evaccordion' );
		}
	};
